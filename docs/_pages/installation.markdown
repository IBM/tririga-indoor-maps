...
1. Add new LocateMap app to Workplace Services UI:
   ...
   1. Open `triview-workplace-services-dev.html` and change.
    {% raw %}   
    ```<tricore-url hidden raw-url="/p/web/locate" bind-url="{{locateMainUrl}}"></tricore-url>``` to ```<tricore-url hidden raw-url="/p/web/locateMap" bind-url="{{locateMainUrl}}"></tricore-url>```  
    {% endraw %}   
